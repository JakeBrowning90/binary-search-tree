...
function driver(){
    // Create random array
    let driverArray = Array.from({length: 20}, () => Math.floor(Math.random() * 100));
    console.log(driverArray);
    // Build new BST with array
    const myTree = new Tree(driverArray);
    // Call isBalanced()
    ...
    // Unbalance by adding new numbers > 100
    myTree.insert(150);
    myTree.insert(125);
    myTree.insert(175);
    // Confirm BST is now unbalanced
    console.log(myTree.isBalanced());
    // Call rebalance()
    myTree.rebalance();
    // Call isBalanced()
